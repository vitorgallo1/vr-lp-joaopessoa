...
- `src/routes/index.tsx` — a página inteira, seção por seção. A vitrine vem do loader,
  não de uma constante.
- `src/routes/admin.tsx` — o painel de estoque. Ver `PAINEL.md`.
- `src/servidor/estoque.ts` — as funções de servidor que a página e o painel chamam.
- `src/lib/motos.ts` — o tipo `Moto`, a validação e a resolução de fotos (URL externa ou
  arquivo do projeto). Roda nos dois lados.
- `src/lib/estoque.server.ts` e `src/lib/sessao.server.ts` — Netlify Blobs e sessão.
  Só servidor: nunca importe de um componente.
- `src/data/catalogo.ts` — o catálogo que a vitrine mostra enquanto o estoque está vazio.
