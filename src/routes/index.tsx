...
// Fotos das motos: 4 por modelo, em src/assets/motos/<modelo>-1..4.webp. São quadros
// do giro 360° do fabricante (recortes com fundo transparente), escolhidos à mão — o
// -1 é sempre o ângulo de perfil. O fundo de estúdio (STUDIO_BG) entra por trás delas.
//
// A LP de Biguaçu carrega as sequências inteiras e oferece o giro 360° no card; aqui
// não, porque o estoque desta unidade vai ser cadastrado por link de foto e sequência
// de giro não sobrevive a isso. Acrescentar uma foto é soltar o arquivo na pasta.
const FOTOS_URLS = import.meta.glob("/src/assets/motos/*.webp", {
  eager: true,
  query: "?url",
  import: "default",
}) as Record<string, string>;

// `numeric` importa para -10 não vir antes de -2 quando um modelo passar de 9 fotos.
const fotos = (modelo: string) =>
  Object.entries(FOTOS_URLS)
    .filter(([path]) => path.includes(`/motos/${modelo}-`))
...
function Lineup() {
  const categories = useMemo(() => ["Todas", ...Array.from(new Set(BIKES.map((b) => b.tag)))], []);
